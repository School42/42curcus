...
/*   Updated: 2025/07/19 15:18:51 by rick             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	perform_dda(t_ray *ray, t_data *data)
{
	while (ray->hit == 0)
	{
		...
		if (ray->map_y < 0.25 || ray->map_y > data->map->height - 0.25 || ray->map_x < 0.25 || ray->map_x > data->map->width - 0.25)
			break;
...
static void	line_height(t_ray *ray, t_data *data, t_player *player)
{
	...
	ray->line_height = (int)(data->win->win_height / ray->wall_dist);
